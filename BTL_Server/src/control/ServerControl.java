...
public class ServerControl {
    ...
    public void ListenConnect() throws IOException{
        while(true){
            ...
            Client client = new Client(info[0],socket.toString(), Integer.parseInt(info[1]), Integer.parseInt(info[2]),info[3], Integer.parseInt(info[4]));
            list.addUser(client);
            for(int i = 0; i < sockets.size(); i++){
                ListUser tmp = new ListUser();
                for(Client j:list.getList()){
                ...
                switch(stt){
                    case 0:{
                        ...
                        for(int i = 0; i < control.getSockets().size(); i++){
                            ListUser tmp = new ListUser();
                            for(Client j:control.getList().getList()){
                    ...
                    case 2:{
                        ...
                        connect.sendInt(1);
                        Client tginfo = connect.revClient();
                        connect.sendInt(1);
                        Client srinfo = connect.revClient();
                        connect.sendInt(1);
                    ...
                    case 3:{
                        ...
                        connect.sendInt(1);
                        Client tinfo = connect.revClient();
                        connect.sendInt(1);
                        Client sinfo = connect.revClient();
                        connect.sendInt(1);
                        String skTxt = sinfo.getSocket().split("\\[")[1].split(",")[2].split("=")[1].split("\\]")[0];
                        for(Client i: control.getList().getList()){

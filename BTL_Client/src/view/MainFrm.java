...
public class MainFrm extends javax.swing.JFrame {
                        ...
                        if(!stop){
                            ...
                            if(list != null){
                                if(list.getList().isEmpty() || list.getList().get(list.getList().size()-1) != null){
                                    ...
                                    Client info = con.rcvClient();
                                    System.out.println(info.getName());
                                    ComingFrm call = new ComingFrm();
                                    call.setMicidx(InputIndx);
                                    call.setSpkidx(OutputIndx);
                                    call.setCon(con);
                                    call.setSourceinfo(new Client(params.getName(),con.getSocket().toString(),listInp.size(), listOup.size()
    ...
    private void jButton2ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton2ActionPerformed
        ...
        if(index != -1){
            Client info = listUserThread.getList().getList().get(index);
